    ...
    if match:
        ...
        if function_name not in dict_content:
            ...
            updated_content = content.replace(
                match.group(0), f"LAMBDA_FUNCTION_NAMES = {{{updated_dict}}}"
            )
...
"""
        ...
        updated_content = content.replace(
            test_match.group(1), test_match.group(1) + new_test
        )
        ...
        if local_test_match:
            new_local_test = (
                f"          python scripts/local_test.py test {function_name}\n"
            )
            updated_content = updated_content.replace(
                local_test_match.group(1), local_test_match.group(1) + new_local_test
            )
        ...
        if deploy_match:
            ...
            updated_content = updated_content.replace(
                deploy_match.group(0), f"for func in {new_funcs}; do"
            )
...
def update_entrypoint_script(function_name):
    ...
    if menu_match:
        ...
        updated_menu = menu_match.group(1).replace(
            'echo "  3) All Functions"',
            f'echo "  3) {function_name}"\necho "  4) All Functions"',
        )
        updated_menu = updated_menu.replace(
            'echo "  4) 🔙 Back to Main Menu"', 'echo "  5) 🔙 Back to Main Menu"'
        )
        ...
        if selection_match:
            new_selection = selection_match.group(1).replace(
                '3) echo "all" ;;',
                f'3) echo "{function_name}" ;;\n        4) echo "all" ;;',
            )
            new_selection = new_selection.replace(
                '4) echo "back" ;;', '5) echo "back" ;;'
            )
            updated_content = updated_content.replace(
                selection_match.group(1), new_selection
            )
        ...
        with open(test_event_file, "w") as f:
            f.write(
                json.dumps(
                    {
                        "httpMethod": "POST",
                        "body": json.dumps({"test": "data", "function": function_name}),
                    },
                    indent=2,
                )
            )
...
def main():
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        ...
        print(
            "Example: python scripts/add_lambda_function.py my_new_function Authentication"
        )
    ...
    print(
        f"2. Add tests to Lambdas/{category}/{function_name}/tests/test_{function_name}.py"
    )
