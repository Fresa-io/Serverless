...
class APIGatewayManager:
    ...
    def create_resource(self, api_id: str, parent_id: str, path_part: str) -> Optional[str]:
    ...
    def add_method(self, api_id: str, resource_id: str, http_method: str, authorization_type: str = 'NONE') -> bool:
    ...
    def add_lambda_integration(self, api_id: str, resource_id: str, http_method: str, 
                             function_name: str, alias: str = None) -> bool:
    ...
    def add_lambda_permission(self, function_name: str, alias: str, api_id: str) -> bool:
        ...
        try:
            ...
            if 'already exists' in str(e):
                print(f"✅ Permission already exists for {function_name}:{alias or 'LATEST'}")
                return True
            else:
                print(f"❌ Error adding Lambda permission: {e}")
                return False
    
    def deploy_api(self, api_id: str, stage_name: str = 'prod', description: str = None) -> bool:
        """Deploy API to stage"""
        try:
    ...
    def create_lambda_api(self, api_name: str, functions_config: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            for func_config in functions_config:
                ...
                staging_func_id = self.create_resource(api_id, staging_id, endpoint_name)
                if staging_func_id:
                    self.add_method(api_id, staging_func_id, 'POST')
                    self.add_lambda_integration(api_id, staging_func_id, 'POST', function_name, 'staging')
                ...
                if prod_func_id:
                    ...
                    self.add_lambda_integration(api_id, prod_func_id, 'POST', function_name, 'prod')
            ...
            for func_config in functions_config:
                endpoint_name = func_config.get('endpoint_name', func_config['function_name'])
    ...
    functions_config = [
        {
            ...
            'endpoint_name': 'recieve-email'
        },
        {
            'function_name': 'signUpCustomer',
            'endpoint_name': 'signup-customer'
        },
        {
            'function_name': 'verifyCodeAndAuthHandler',
            'endpoint_name': 'verify-code'
        },
        {
            'function_name': 'identity_provider_auth',
            'endpoint_name': 'identity-auth'
        },
        {
            'function_name': 'social_auth_user',
            'endpoint_name': 'social-auth'
        },
        {
            'function_name': 'defineAuthChallenge',
            'endpoint_name': 'define-challenge'
        },
        {
            'function_name': 'verifyAuthChallenge',
            'endpoint_name': 'verify-challenge'
        },
        {
            'function_name': 'createAuthChallenge',
            'endpoint_name': 'create-challenge'
        }
    ]
