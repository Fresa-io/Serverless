...
from services.ses.create_welcome_template import (
    create_welcome_template_with_logo,
    get_gendered_template_data,
)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  python services/ses/template_manager.py create <template_name> <subject> <html_content> [text_content]"
        )
        print(
            "  python services/ses/template_manager.py update <template_name> <subject> <html_content> [text_content]"
        )
    ...
    if command == "list":
        ...
        if len(sys.argv) < 5:
            print(
                "❌ create command requires: template_name subject html_content [text_content]"
            )
            sys.exit(1)
        ...
        if len(sys.argv) < 5:
            print(
                "❌ update command requires: template_name subject html_content [text_content]"
            )
            sys.exit(1)
