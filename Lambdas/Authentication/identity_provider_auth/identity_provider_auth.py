    ...
    try:
        ...
        try:
            ...
            if not auth_result.get('success'):
                return {'statusCode': 500, 'headers': headers, 'body': json.dumps({'error': auth_result.get('error')})}

            # Build user info from Cognito attributes
            user_attributes = {attr['Name']: attr['Value'] for attr in user_details['UserAttributes']}
            ...
            if e.response['Error']['Code'] == 'UserNotFoundException':
                logger.info(f"User {email} not found. Signaling client to proceed with creation.")
