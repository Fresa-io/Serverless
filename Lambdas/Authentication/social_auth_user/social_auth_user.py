...
def verify_google_token(id_token_string):
    ...
    try:
        ...
        if 'error' in token_info:
            return {'success': False, 'error': token_info.get('error_description', 'Invalid token')}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if provider not in ['google', 'facebook']:
            return {'statusCode': 400, 'headers': headers, 'body': json.dumps({'error': 'Invalid provider.'})}
        ...
        if not token:
            return {'statusCode': 400, 'headers': headers, 'body': json.dumps({'error': 'Missing access/id token.'})}

        # Get additional profile data from request
        gender = body.get('gender')
        birthdate = body.get('birthdate') or body.get('dateOfBirth')  # Support both field names
        ...
        if not all([gender, birthdate]):
            return {'statusCode': 400, 'headers': headers, 'body': json.dumps({'error': 'Missing required fields: gender, birthdate/dateOfBirth.'})}
        ...
        if not user_info.get('success'):
            return {'statusCode': 401, 'headers': headers, 'body': json.dumps({'error': user_info.get('error', 'Token verification failed.')})}
        ...
        if not email:
            return {'statusCode': 400, 'headers': headers, 'body': json.dumps({'error': 'Email not provided by social provider.'})}
        ...
        try:
            ...
            logger.warning(f"User {email} already exists but Lambda 2 was called. This indicates a race condition.")
            return {'statusCode': 409, 'headers': headers, 'body': json.dumps({'error': 'User already exists.'})}
        except ClientError as e:
            if e.response['Error']['Code'] != 'UserNotFoundException':
                # Some other error occurred
                logger.error(f"Error checking user existence: {str(e)}")
                return {'statusCode': 500, 'headers': headers, 'body': json.dumps({'error': f'Error checking user: {str(e)}'})}
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'UsernameExistsException':
                ...
                logger.warning(f"User {email} was created by another process. Attempting to authenticate instead.")
            else:
                logger.error(f"Error creating Cognito user: {str(e)}")
                return {'statusCode': 500, 'headers': headers, 'body': json.dumps({'error': f'Could not create user: {str(e)}'})}
        ...
        try:
            ...
            return {'statusCode': 500, 'headers': headers, 'body': json.dumps({'error': f'User created but authentication failed: {str(e)}'})}
