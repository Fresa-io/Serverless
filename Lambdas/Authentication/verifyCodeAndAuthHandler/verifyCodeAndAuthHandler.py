...
CODE_EXPIRATION_MINUTES = int(os.environ.get('CODE_EXPIRATION_MINUTES', '5'))  # Default 5 minutes
...
def validate_code_in_dynamodb(email, code):
    ...
    try:
        ...
        if 'Item' not in response:
            return {'valid': False, 'error': 'Verification code has expired', 'status_code': 401}
    ...
    try:
        ...
        try:
            ...
            if error_code == 'NotAuthorizedException':
                return {
                    'statusCode': 401,
                    'body': json.dumps({'error': 'Invalid code or authentication failed'})
                }
                ...
                return {
                    'statusCode': 500,
                    'body': json.dumps({'error': 'Authentication service configuration error'})
                }
            else:
                return {
                    'statusCode': 401,
