...
def validate_function_name(name):
    ...
    if not name.replace("_", "").replace("-", "").isalnum():
        return (
            False,
            "Function name must contain only letters, numbers, underscores, and hyphens",
        )
...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "scripts/add_lambda_function.py", function_name, category],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0:
            ...
            print("\n📋 Next steps:")
            print(
                f"1. Edit the function code: Lambdas/{category}/{function_name}/{function_name}.py"
            )
